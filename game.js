...
// Карты (те же, что и раньше)
const maps = [ ... ]; // Вставь сюда полный массив карт из предыдущего кода
...
class Bonus {
    constructor(x, y, type) {
        this.x = x;
        this.y = y;
        this.type = type; // 'speed' или 'shield'
    }
}
        ...
        for (let id in players) {
            ...
            if (p.x > tank.x && p.x < tank.x + TILE_SIZE &&
                ...
                if (!tank.shield) {
                    ...
                    if (tank.health <= 0) {
                        gameActive = false;
                        // Здесь можно вызвать колбэк окончания игры
                    }
                }
                projectiles.splice(i, 1);
                break;
